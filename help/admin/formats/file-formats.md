...
列出可用於建立[!DNL FTP]基於資料檔案的宏。 某些宏可用於所有資料檔案欄位和行。 其他宏僅特定於標題行和資料行。

## 常見宏{#common-macros}

這些宏可用於任何格式欄位。 如需範例，請參閱[檔案格式巨集範例](../formats/file-format-examples.md)。
...
<table id="table_A3309E175ABF4651BD11CE3632B3C553"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>訂單／目標ID的別名。 </p> <p>此別名的值設定在目標的<span class="wintitle">外來帳戶ID </span>欄位中（在<span class="wintitle">基本設定</span>部分）。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>10位數的UTC、Unix時間戳記。 </p> <p>它也可以按照Java日期／時間戳記格式規則格式化為<code>YYYYMMDDhhmmss</code>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
僅用於標題欄位的宏。 如需範例，請參閱[檔案格式巨集範例](../formats/file-format-examples.md)。
...
僅用於資料行的宏。 如需範例，請參閱[檔案格式巨集範例](../formats/file-format-examples.md)。
...
<table id="table_E378F94A3907407AA8110C8EE6C10909"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> <span class="term"> 資料合作夥伴唯一使用者識別 </span>碼。如果該ID已與<span class="keyword"> Audience Manager </span>裝置ID同步，則傳回您指派給使用者／網站訪客的ID。 </p> <p>如果DPID為0，此巨集會傳回<span class="keyword"> Audience Manager </span> ID，而非使用者的ID。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>使用條件<code>if</code>和<code>SEGMENT_LIST</code>和<code>REMOVED_SEGMENT_LIST</code>宏時需要。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code>if(SEGMENT_LIST &amp;&amp; REMOVED_SEGMENT_LIST)endif</code> </p> </td> 
   <td colname="col2"> <p>此宏組合會建立條件語句，其中列出用戶屬於<i>且</i>且已從中刪除的段。 如果兩個條件都不符合或沒有資料，則會傳回空字串。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>合作夥伴ID(PID)。 PID會顯示在管理UI的<span class="wintitle">描述檔</span>標籤下。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>傳回清單中的區段清單。 接受下列可選變數： </p> 
    <ul id="ul_B111AA0D6C18445598A1444B8B7E9325"> 
     <li id="li_8603B40229624856AF1FBC434DB8F16A"> <code>segmentId</code>:舊版ID。不再提倡。使用<code>sid</code>（僅小寫）。 </li> 
     <li id="li_1EF40DDCA3C5447586904CF021D8F912"> <code>csegid</code>:舊版ID。不再提倡。使用<code>sid</code>（僅小寫）。 </li> 
     ...
     <li id="li_FE5049089F2944FA9DB9F9D546DBA167"> <code>alias</code>:區段的對應。不再提倡。使用<code>sid</code>（僅小寫）。 </li> 
